...
- [ ] As to algorithm selection: The algorithm underlying an approach (e.g., the metaheuristic) should be justified and appropriate for the problem being optimized. For example, do not use meta-heuristic methods such as Simulated Annealing, NSGA-II, etc. to solve an optimization problem unless it is actually the appropriate approach for the optimization problem.
